...
if(isset($_POST["action"]))
{
	...
	if($_POST['action'] == 'patient_register')
	{
		...
		else
		{
			...
			$data = array(
				...
				':patient_mid_name'				=>	$object->clean_input($_POST["patient_mid_name"]),
			...
			(patient_email_address, patient_password, patient_first_name, patient_last_name, patient_mid_name, patient_suffix, patient_date_of_birth, patient_gender, patient_address, patient_phone_no, patient_maritial_status, patient_added_on, patient_verification_code, email_verify) 
			VALUES (:patient_email_address, :patient_password, :patient_first_name,  :patient_last_name, :patient_mid_name, :patient_suffix, :patient_date_of_birth, :patient_gender, :patient_address, :patient_phone_no, :patient_maritial_status, :patient_added_on, :patient_verification_code, :email_verify)
			";
			...
			$mail->Port = '2525';
			$mail->SMTPAuth = true;
			$mail->Username = 'capstone02pdh@gmail.com';
			$mail->Password = 'BF577B8099794262AF85EAF6111B8D35FC6F';
			$mail->SMTPSecure = '';
			$mail->From = 'capstone02pdh@gmail.com';
			$mail->FromName = 'PDH';
			...
			<p>Sincerely,</p>
			<p>PDH-OutPatient-Scheduling-System</p>
			';
	...
	if($_POST['action'] == 'fetch_schedule')
	{
		...
		foreach($result as $row)
		{
			...
			$sub_array[] = '
			<div align="center">
			<button type="button" name="get_appointment" class="btn btn-primary btn-sm get_appointment"
			 data-doctor_id="'.$row["doctor_id"].'"
			 data-doctor_schedule_date="'.$row["doctor_schedule_date"].'"
			data-doctor_schedule_id="'.$row["doctor_schedule_id"].'">Get Appointment</button>
			</div>
			';
			$data[] = $sub_array;
		}
	...
	if($_POST['action'] == 'edit_profile')
	{
		$data = array(
			...
			':patient_mid_name'			=>	$_POST["patient_mid_name"],
		...
		patient_mid_name = :patient_mid_name,
	...
	if($_POST['action'] == 'book_appointment')
	{
		...
		//validate patient schedule same day: change the logic if needed 
		$object->query = "
		SELECT * FROM appointment_table a
		inner join doctor_schedule_table b on b.doctor_schedule_id = a.doctor_schedule_id
		WHERE a.patient_id = :patient_id 
		and b.doctor_schedule_date  = '".$_POST['hidden_doctor_schedule_date']."'
		";

		$object->execute($dataSched);
		if($object->row_count() > 0)
		{
	...
	if(isset($_POST['action']) && $_POST['action'] == 'change_password')
	{
		...
		UPDATE patient_table  
		SET patient_password = :patient_password 
		WHERE patient_email_address = '".$_SESSION['email']."'
		";
	
		$object->execute($data);
	
		$_SESSION['success_message'] = '<div class="alert alert-success">Change Password Success</div>';
