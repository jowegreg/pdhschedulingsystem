...
if(isset($_POST["action"]))
{
	if($_POST["action"] == 'fetch')
	{
		...
		if($_SESSION['type'] == 'Admin')
		{
			...
			{
			 	$search_query .= 'WHERE doctor_schedule_table.doctor_schedule_date 
					BETWEEN "'.$_POST["start_date"].'" AND "'.$_POST["end_date"].'" AND (';
			}
			else
			{
				$search_query .= 'WHERE ' ;
	...
	if($_POST['action'] == 'change_appointment_status')
	{
		if($_SESSION['type'] == 'Admin')
		{
			...
			// Check if patient_come_into_hospital is 'NO', then set status to 'Cancelled'
			if ($patient_come_into_hospital == 'No') {
				$status = 'Cancel';
			}
		...
		else
		{
			...
			echo '<div class="alert alert-success">Appointment Status change to Cancelled</div>';
		}
